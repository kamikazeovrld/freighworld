					...
					if ( class_exists( 'Redux_VendorURL' ) ) {
						...
						if ( ! self::is_enqueued( $handle, 'enqueued', $is_script ) ) {
							...
							if ( self::$_parent->args['dev_mode'] ) {
								$msg = sprintf( wp_kses(__( 'If you are developing offline, please download and install the <a href="%s" target="_blank">Redux Vendor Support</a> plugin/extension to bypass the our CDN and avoid this warning', 'redux-framework' ),$allowed_html), 'https://github.com/reduxframework/redux-vendor-support' );
							}
		...
		private static function _vendor_plugin( $register = true, $handle, $src_cdn, $deps, $ver, $footer_or_media, $is_script = true ) {
            ...
            if ( class_exists( 'Redux_VendorURL' ) ) {
				...
				if ( ! self::$_set ) {
					self::$_parent->admin_notices[] = array(
						'type'    => 'error',
						'msg'     => sprintf( wp_kses(__( 'The <a href="%s">Vendor Support plugin</a> (or extension) is either not installed or not activated and thus, some controls may not render properly.  Please ensure that it is installed and <a href="%s">activated</a>', 'redux-framework' ),$allowed_html), 'https://github.com/reduxframework/redux-vendor-support', admin_url( 'plugins.php' ) ),
						'id'      => $handle . '23',
						'dismiss' => false,
					);
					self::$_set = true;
