...
<div class="wrap about-wrap redux-status">
	...
	<table class="redux_status_table widefat" cellspacing="0" id="status">
		...
		<tr>
			...
			<td><?php
				if ( $sysinfo['redux_data_writeable'] == 'true' ) {
					...
					printf( '<mark class="error">' . '&#10005; ' .wp_kses(__( 'To allow data saving, make <code>%s</code> writable.', 'redux-framework' ) . '</mark>', $sysinfo['redux_data_dir'] ),$allowed_html);
				}
				?></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td><?php
				...
				if ( $memory < 40000000 ) {
					echo '<mark class="error">' . sprintf( wp_kses(__( '%s - We recommend setting memory to at least 40MB. See: <a href="%s" target="_blank">Increasing memory allocated to PHP</a>', 'redux-framework' ),$allowed_html), $sysinfo['wp_mem_limit']['size'], 'http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP' ) . '</mark>';
				} else {
	...
	<table class="redux_status_table widefat" cellspacing="0" id="status">
		...
		<tr>
			<td data-export-label="Localhost Environment"><?php esc_html_e( 'Localhost Environment', 'redux-framework' ); ?>:
			</td>
		...
		if ( $sysinfo['dom_document'] == true ) {
			...
			$posting['dom_document']['note']    = sprintf( wp_kses(__( 'Your server does not have the <a href="%s">DOMDocument</a> class enabled - HTML/Multipart emails, and also some extensions, will not work without DOMDocument.', 'redux-framework' ),$allowed_html), 'http://php.net/manual/en/class.domdocument.php' ) . '</mark>';
		}
		*/
		//// GZIP
		...
		//    $posting['gzip']['note']    = sprintf( wp_kses__( 'Your server does not support the <a href="%s">gzopen</a> function - this is required to use the GeoIP database from MaxMind. The API fallback will be used instead for geolocation.', 'redux-framework' ),$allowed_html), 'http://php.net/manual/en/zlib.installation.php' ) . '</mark>';
		//}
	...
	<table class="redux_status_table widefat" cellspacing="0" id="status">
		...
		<tr>
			...
			<td><?php
				echo is_child_theme() ? '<mark class="yes">' . '&#10004;' . '</mark>' : '&#10005; <br /><em>' . sprintf( wp_kses(__( 'If you\'re modifying Redux Framework or a parent theme you didn\'t build personally, we recommend using a child theme. See: <a href="%s" target="_blank">How to create a child theme</a>', 'redux-framework' ),$allowed_html), 'http://codex.wordpress.org/Child_Themes' ) . '</em>';
				?></td>
		</tr>
		<?php
		if ( is_child_theme() ) {
