 ...
 *
 */
function webuild_post_gallery_html( $gallery_html, $attr ) {
	if ( isset( $attr['protype'] ) ) {
		switch ( $attr['protype'] ) {
			...
			case 'gallery_nearby':
				//printr($gallery_html);
				$gallery_html = webuild_gallery_royalslider( $attr );
				//printr($gallery_html);
				break;
			case 'gallery_lightbox':
				$gallery_html = webuild_gallery_fancybox( $attr );
				break;
			default:
				break;
		}
...
add_filter( 'post_gallery', 'webuild_post_gallery_html', 10, 2 );
if ( ! function_exists( 'webuild_gallery_royalslider' ) ) {
	function webuild_gallery_royalslider( $attr ) {
		// get post
		$post = get_post();
		...
		$slider_settings = apply_filters( 'webuild_gallery_slider_settings', $slider_settings, $id );
...
if ( ! function_exists( 'webuild_gallery_fancybox' ) ) {
	function webuild_gallery_fancybox( $attr ) {
		$post = get_post();
		// 'ids' is explicitly ordered, unless you specify otherwise.
		if ( ! empty( $attr['ids'] ) ) {
			if ( empty( $attr['orderby'] ) ) {
