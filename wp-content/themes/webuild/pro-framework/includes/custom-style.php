...
CSS;
	...
	$bg_css = webuild_redux_bg_css( $options['background-image-footer-area'] );
...
CSS;
	...
	if ( $apro_options['container-width'] ) {
		echo <<<CSS
	...
	@media (min-width: {$options['container-width']['width']}) {
		...
		.container {
			...
			width:{$apro_options['container-width']['width']};
...
CSS;
	...
	return webuild_css_compress( $css );
}
...
CSS;
	...
	if ( ! $options['background-image-page-title']['background-image'] ) {
		echo <<<CSS
...
CSS;
	...
	if ( $options['content-offset'] && $options['transparent-header'] ) {
		if ( $options['slide-template'] != 'no-slider' ) {
			echo <<<CSS
	.mobile-menu-cont {
		margin-bottom: {$options['content-offset']['height']} !important;
	}	
	.pro-slider {
