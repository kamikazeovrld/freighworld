...
if ( ! class_exists( 'Walker_Nav_Menu_Custom' ) ) {
	...
	class Walker_Nav_Menu_Custom extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( $depth == 0 ) {
				if ( middle_logo() && $apro_options['logo-after'] == $this->counter ) {
			...
			if ( $depth == 1 && isset( $this->menu_type[ $item->menu_item_parent ] ) ) {
				$bs_col = webuild_get_bootstrap( $this->child_count );
			}
			...
			if ( empty( $item->column_title ) ) {
				...
				// adding icon
				$item_output .= ( ! empty( $item->icon ) ) ? '<i class="' . webuild_icon_class( $item->icon ) . '"></i>' : '';
				$item_output .= $args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after;
				if ( $item->has_children && $depth == 0 && $apro_options['menu-dropdown-icon'] ) {
	...
	class Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu {
		...
		<li id="menu-item-<?php echo esc_attr( $item_id ); ?>" class="<?php echo implode( ' ', $classes ); ?>">
          ...
          <span class="item-controls">
            ...
            <!-- Mega Labels Action -->
	          <?php do_action( 'webuild_mega_menu_labels' ); ?>
	          <!-- /Mega Labels Action -->
			...
			<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr( $item_id ); ?>">
				...
				<!-- Mega Menu Fields-->
				<?php do_action( 'webuild_mega_menu_fields', $item_id, $item ); ?>
				<!-- /Mega Menu Fields-->
