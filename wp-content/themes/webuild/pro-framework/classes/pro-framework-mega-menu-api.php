 ...
 *
 */
include_once( get_template_directory() . '/pro-framework/includes/walker_nav_menu.php');

class PROFramework_Mega_Menu_API extends PROFramework_Abstract {
	...
	public function __construct() {
		...
		$this->addAction( 'webuild_mega_menu_fields', 'webuild_mega_menu_fields', 10, 2 );
		$this->addAction( 'webuild_mega_menu_labels', 'webuild_mega_menu_labels' );
	}
	 ...
	 */
	public function webuild_mega_menu_fields( $item_id, $item ) {
		...
		<div class="field-icon description description-wide">
			...
			$icon = ( ! empty( $item->icon ) ) ? ' class="' . webuild_icon_class( $item->icon ) . '"' : '';
			?>
			<div class="webuild_field webuild_field_icon">
	...
	public function webuild_mega_menu_labels() {
