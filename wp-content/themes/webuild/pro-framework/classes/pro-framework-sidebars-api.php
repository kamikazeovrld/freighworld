 ...
 *
 */
class PROFramework_Sidebars_API extends PROFramework_Abstract {
	...
	public function register_default_sidebars() {
		...
		foreach ( $defaults as $key => $name ) {
			register_sidebar( array(
				...
				'before_widget' => '<div class="webuild_widget %2$s">',
				'after_widget'  => '</div>',
	...
	public function register_footer_sidebars() {
		global $apro_options;
		$sidebars = isset( $apro_options['footer-widgets'] ) ? $apro_options['footer-widgets'] : array();
		if ( $sidebars ) {
			switch ( $sidebars ) {
				case 5:
					$length = 6;
	...
	public function register_custom_sidebars() {
		global $apro_options;
		$get_custom_sidebars = isset( $apro_options['custom-sidebars'] ) ? $apro_options['custom-sidebars'] : array();
