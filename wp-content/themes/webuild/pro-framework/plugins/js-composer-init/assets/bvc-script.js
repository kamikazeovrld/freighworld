...
;(function ( $, window, document, undefined ) {
    ...
    if (window.VcColumnView) {
        ...
        window.PROCarouselView = window.VcColumnView.extend({
            ...
            addDirectlyElement: function (e) {
                e.preventDefault();
                var carousel = Shortcodes.create({shortcode: 'webuild_carousel_item', parent_id: this.model.id});
                return carousel;
            },
        ...
        window.PROFAQSView = window.VcTabsView.extend({
            ...
            addTab: function (e) {
                ...
                var tabs_count = this.$tabs.find('[data-element_type=webuild_faq_block]').length,
                    tab_title = 'FAQ ' + (tabs_count + 1),
                    tab_id = (+new Date() + '-' + tabs_count + '-' + Math.floor(Math.random() * 11));

                var faq = vc.shortcodes.create({ shortcode: 'webuild_faq_block', params: {title: tab_title, tab_id: tab_id}, parent_id: this.model.id });
            ...
            cloneModel: function (model, parent_id, save_order) {
                ...
                if (model.get('shortcode') === 'webuild_faq_block') {
                    tab_id = (+new Date() + '-' + this.$tabs.find('[data-element-type=webuild_faq_block]').length + '-' + Math.floor(Math.random() * 11) );
                    _.extend(new_params, {tab_id: tab_id});
                }

                model_clone = Shortcodes.create({ shortcode: model.get('shortcode'), id: vc_guid(), parent_id: parent_id, order: new_order, cloned: (model.get('shortcode') === 'webuild_faq_block' ? false : true), cloned_from: model.toJSON(), params: new_params });
        ...
        window.PROFAQView = window.VcTabView.extend({
                ...
                cloneModel: function (model, parent_id, save_order) {
                    ...
                    if (model.get('shortcode') === 'webuild_faq_block') {
                        tabs_count = this.$tabs.find('[data-element_type=webuild_faq_block]').length;
                        tab_id = (+new Date() + '-' + tabs_count + '-' + Math.floor(Math.random() * 11) ),
                            tab_title = 'FAQ ' + (tabs_count + 1),
    ...
    // -------------------------------------------------------------------------
    _.extend(vc.atts, {
        vc_webuild_exploded_textarea: {
            parse: function (param) {
                var $field = this.content().find('.wpb_vc_param_value[name=' + param.param_name + ']');
                return $field.val().replace(/\n/g, '~');
            }
        },
        vc_webuild_style_textarea: {
            parse: function (param) {
                var $field = this.content().find('.wpb_vc_param_value[name=' + param.param_name + ']');
                return $field.val().replace(/\n/g, '');
            }
