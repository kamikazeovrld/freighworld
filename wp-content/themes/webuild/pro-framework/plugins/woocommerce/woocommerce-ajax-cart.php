	...
	<ul class="cart_list cart-items-container">
		<?php if ( sizeof( $woocommerce->cart->get_cart() ) > 0 ) : ?>
			<li class="header"><span><?php echo sizeof( $woocommerce->cart->get_cart() );
					esc_html_e( ' items', 'woocommerce' ); ?></span><?php esc_html_e( ' in the shopping cart', 'woocommerce' ); ?> </li>
			<?php foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) :
				...
				<li class="item-product-cart">
					...
					<?php echo sprintf( '<a href="javascript:void(0);" rel="' . $cart_item_key . '" class="ajax-remove-item remove remove-product" title="%s"><i class="fa fa-times"></i></a>', esc_html__( 'Remove this item', 'woocommerce' ) ); ?>
				</li>
			<?php endforeach; ?>
			...
			<li class="buttons">
				<a class="btn btn-view"
				   href="<?php echo $woocommerce->cart->get_cart_url(); ?>"><?php esc_html_e( 'View Cart', 'woocommerce' ); ?></a>
				<a class="btn btn-checkout"
				   href="<?php echo $woocommerce->cart->get_checkout_url(); ?>"><?php esc_html_e( 'Checkout', 'woocommerce' ); ?></a>
			</li>
		<?php else : ?>
			<li class="header"><?php esc_html_e( 'No products in the cart.', 'woocommerce' ); ?></li>
		<?php endif; ?>
	</ul><!-- end product list -->
