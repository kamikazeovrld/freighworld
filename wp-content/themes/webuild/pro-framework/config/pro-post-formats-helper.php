 ...
 *
 */
if ( ! function_exists( 'webuild_get_first_url_from_string' ) ) {
	function webuild_get_first_url_from_string( $string ) {
 ...
 *
 */
if ( ! function_exists( 'webuild_get_shortcode_regex' ) ) {
	function webuild_get_shortcode_regex( $tagregexp = '' ) {
 ...
 *
 */
if ( ! function_exists( 'webuild_tagregexp' ) ) {
	function webuild_tagregexp() {
		return apply_filters( 'webuild_custom_tagregexp', 'video|audio|playlist|video-playlist|embed|webuild_media' );
	}
}
/**
 *
 ...
 *
 */
if ( ! function_exists( 'webuild_post_format_link_helper' ) ) {
	function webuild_post_format_link_helper( $content = null, $title = null, $post = null ) {
		if ( ! $content ) {
			$post = get_post( $post );
			$title = $post->post_title;
			$content = $post->post_content;
		}
		$link = webuild_get_first_url_from_string( $content );
		if ( ! empty( $link ) ) {
 ...
 *
 */
if ( ! function_exists( 'webuild_get_link_attributes' ) ) {
	function webuild_get_link_attributes( $string ) {
 ...
 *
 */
if ( ! function_exists( 'webuild_post_media_responsive' ) ) {
	function webuild_post_media_responsive( $html ) {
		$output = ( strpos( $html, '<iframe' ) !== false ) ? '<div class="pro-fluid"><div class="pro-fluid-inner">' . $html . '</div></div>' : $html;
 ...
 *
 */
if ( ! function_exists( 'webuild_post_media' ) ) {
	function webuild_post_media( $content ) {
 ...
 *
 */
if ( ! function_exists( 'webuild_post_gallery' ) ) {
	function webuild_post_gallery( $content ) {
		global $apro_options;
		$pattern = webuild_get_shortcode_regex( 'gallery' );
		preg_match( '/' . $pattern . '/s', $content, $media );
		if ( ! empty( $media[2] ) && $apro_options['blog_featured_image'] ) {
 ...
 *
 */
function webuild_string_limit_words( $string, $word_limit ) {
